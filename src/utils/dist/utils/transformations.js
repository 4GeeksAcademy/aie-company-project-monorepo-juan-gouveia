...
export function convertCurrency(amount, fromCurrency, toCurrency) {
    ...
    const converted = fromCurrency === "USD" ? amount * 4000 : amount / 4000;
...
export function scoreLocationPerformance(location, sales, wasteRecords, menuItems) {
    const locationCurrency = location.country === "Colombia" ? "COP" : "USD";
...
export function countSalesByPaymentMethod(sales) {
    const initialCounts = { Cash: 0, "Credit card": 0, "Debit card": 0, "Digital wallet": 0, };
...
export function findTopSellingItems(sales, menuItems, topN) {
    ...
    return Array.from(totalSoldByItemId.entries()).map(([itemId, totalSold]) => ({ item: itemById.get(itemId), totalSold, })).sort((a, b) => b.totalSold - a.totalSold).slice(0, Math.max(0, topN));
}
// Función para agrupar registros de desperdicio por razón
export function groupWasteByReason(wasteRecords) {
    const initialGroups = { Expired: [], "Cooking error": [], "Customer return": [], Damage: [], Other: [], };
    return wasteRecords.reduce((groups, wasteRecord) => {
        groups[wasteRecord.reason].push(wasteRecord);
        return groups;
    }, initialGroups);
}
...
export function calculateCountryComparison(sales, locations, menuItems) {
    ...
    sales.forEach((sale) => {
        ...
        const hasValidMenuItem = validMenuItemIds.has(sale.itemId);
